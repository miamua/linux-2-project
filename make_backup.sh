...
function check_backup_loc {
	...
	then
		echo "Please spectify the full path of where to send the backup to, by creating the backup_loc.conf file in the root directory"
		exit 1
	fi
}
